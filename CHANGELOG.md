...
> WongMod 为 **Rust 全量重构** 的多用户服务端。两者定位不同：原版偏轻量客户端（可选全局 Web 密码，或平级多账号），WongMod 偏可部署的多人消息管理平台。

| 维度 | 原版（Python） | WongMod（Rust） |
|------|----------------|-----------------|
| 运行形态 | 需 Python 3.7+ 与 `pip install qrcode pycryptodomex pilk` | 单二进制 + `web/` 目录，零运行时依赖 |
| 用户体系 | 可选全局 Web 密码（单用户模式），或平级多账号（每账号一个微信 bot，无角色、无配额） | owner / admin / user 三级角色，邀请码注册、设备令牌、会话管理 |
| 存储 | JSON 文件（config / messages / media_cache 散落目录） | SQLite（system.db + 用户库），配额、LRU 媒体清理 |
...
| 能力 | 原版 | WongMod 现状 |
|------|------|--------------|
| AI 能力全家桶 | 自动回复（gpt-3.5-turbo）、识图（gpt-4o）、生图（dall-e-3）、文件识别（PDF/DOCX/XLSX/PPTX）、每用户提示词、媒体记忆、定时主动消息、工具调用 | 不内置，计划以 Webhook + 外部机器人配置提供 |
| 邮箱验证与找回 | 邮箱注册/绑定、验证码 + 邮件找回密码（仅适配 QQ 邮箱，直连 QQ MX） | 邮箱仅作联系方式字段，密码重置走管理员 CLI |
...
| 能力 | 说明 |
|------|------|
| 三级角色 + 邀请码 + 配额 | owner / admin / user、可过期邀请码、5 维配额 + 3 项功能开关 |
| WebDAV 媒体外置 | 媒体迁移到用户自有 WebDAV（省磁盘），流量节省模式、迁移进度 ETA |
| Webhook 出站 | 收消息实时转发到自定义 URL（HMAC-SHA256 签名、SSRF 防护）——预留的 AI / 机器人扩展点 |
| WebSocket 实时推送 | 替代 500ms 轮询与 SSE，含发送回执事件 |
| 发送可靠性 | 发送状态机 + 5 次指数退避重试、崩溃后恢复、失败手动补发 |
| 会话与设备管理 | 登录防固定、改密/禁用即全局吊销、remember-me 设备令牌可单独撤销 |
| 审计日志（90 天） | 管理操作全量记录 |
| 安全加固 | PBKDF2 600k、AES-256-GCM 凭证加密、全端点限流、CSRF/Origin 校验、SSRF DNS 固定、公网绑定启动守卫 |
